...
export async function PATCH(
  ...
  if (
    item.donor.toString() !== decoded.userId &&
    user.role !== "admin" &&
    user.role !== "superadmin"
  ) {
    return NextResponse.json(
      { error: "Not permitted to upload images for this item" },
      { status: 403 }
    );
  }
  ...
  const imageUrl = `/uploads/items/${params.id}/${fileName}`;
  item.images.push(imageUrl);
  await item.save();

  return NextResponse.json(
    { message: "Image added", imageUrl },
    { status: 200 }
  );
}
